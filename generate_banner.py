...
def scene_neuron(t):
    ...
    The arbor is grown in polar coordinates: five primary dendrites, each
    owning an angular wedge of its own, and every fork splits its parent's
    wedge in two with a gap left between. Branches therefore only ever meet at
    the fork that made them — no limb crosses, touches or welds into a
    neighbour, and the gaps widen with radius so the twigs stay legible. Every
    limb is a tapered filled ribbon: a centreline offset perpendicular by a
    width that shrinks toward the tip, thinning at each fork the way Rall's
    rule says a real dendrite does. Soma, dendrites and axon are painted in two
    passes — a fattened outline, then a fill on top — so the cell reads as one
    continuous silhouette with no seams where a dendrite meets the cell body.
    The axon leaves through a hillock, runs under myelin sleeves broken by
    nodes of Ranvier, and fans into a terminal arbor of boutons.
    """
    a = t["magenta"]
    ...
    def ribbon(pts, w0, w1, prof=None):
        ...
        for i, (x, y) in enumerate(pts):
            ...
            nx, ny = -dy / L, dx / L
            u = i / (n - 1)
            w = (prof(u) if prof else w1 + (w0 - w1) * (1 - u) ** 1.3) / 2
    ...
    def arc(ox, oy, r0, r1, th0, th1, bow, n=7):
        """A branch centreline in polar space — it only ever grows outward.

        Turning is spent on easing from the parent's bearing to its own, so a
        fork opens cleanly and the two daughters never wander back together.
        """
        pts = []
        for i in range(n + 1):
            u = i / n
            s = u * u * (3 - 2 * u)
            th = th0 + (th1 - th0) * s + bow * math.sin(3.14159 * u)
            r = r0 + (r1 - r0) * u
            pts.append((round(ox + math.cos(th) * r, 1),
                        round(oy + math.sin(th) * r, 1)))
        return pts

    def limb(r0, seg, th0, lo, hi, w0, depth, chain):
        """One branch aimed somewhere inside its wedge; daughters split it.

        The wedge is what keeps the arbor legible: a branch may wander, fork
        early or fork late, but it can never leave the slice of the circle it
        was handed, and a gap is carved out between the daughters' slices.
        """
        th1 = lo + (hi - lo) * rng.uniform(0.38, 0.62)
        r1 = r0 + seg
        w1 = max(w0 * 0.6, 1.0) if depth else 1.0
        pts = arc(sx0, sy0, r0, r1, th0, th1,
                  rng.uniform(-0.12, 0.12) * (hi - lo))
        shapes.append(ribbon(pts, w0, w1))
        chain = chain + (pts[1:] if chain else pts)
        if depth == 0:
            tips.append(chain)
            return
        g = (hi - lo) * 0.2                      # the clear air between daughters
        mid = lo + (hi - lo) * rng.uniform(0.42, 0.58)
        for a0, a1 in ((lo, mid - g), (mid + g, hi)):
            limb(r1, seg * (0.78 if depth > 1 else 0.95) * rng.uniform(0.88, 1.1),
                 th1, a0, a1, w1, depth - 1, chain)

    # five primaries, twenty twigs — every wedge stays 14° clear of the next
    for c in (300, 252, 204, 156, 108):
        h, th0 = math.radians(17), math.radians(c)
        limb(12, 28 + rng.uniform(-3, 3), th0, th0 - h, th0 + h, 11.5, 2, [])

    soma = []
    for i in range(60):
        th = i / 60 * 6.28319
        rr = 2.2 * math.sin(3 * th + 0.6) + 1.4 * math.sin(5 * th + 1.9)
        soma.append((round(sx0 + math.cos(th) * (23 + rr), 1),
                     round(sy0 + math.sin(th) * (19 + rr), 1)))
    shapes.append(soma)

    axon = bez((sx0 + 11, sy0 + 7), (222, 294), (302, 298), (378, 332), 44)
    shapes.append(ribbon(axon[:12], 13, 4.6))            # hillock + initial seg
    shapes.append(ribbon(axon[10:], 4.6, 3.8))           # constant-calibre axon

    # the terminal fan gets the same wedge treatment: six twigs, 24° apart
    ex, ey = axon[-1]
    for c in (-24, 24, 72):
        th0 = math.radians(c)
        shapes.append(ribbon(arc(ex, ey, 3, 22, th0, th0, 0.0, 5), 4.4, 3.0))
        for k in (-1, 1):
            th1 = math.radians(c + k * 12)
            r = 46 + rng.uniform(-3, 3)
            shapes.append(ribbon(arc(ex, ey, 22, r, th0, th1, 0.0, 5), 3.0, 1.9))
            boutons.append((round(ex + math.cos(th1) * r, 1),
                            round(ey + math.sin(th1) * r, 1),
                            round(rng.uniform(2.9, 3.6), 1)))
    ...
    internodes = [axon[i:i + 6] for i in range(5, len(axon) - 6, 7)]
    for seg in internodes:
        pg = ribbon(seg, 0, 0,
                    prof=lambda u: 10.6 * (0.6 + 0.4 * math.sin(3.14159 * u) ** 0.5))
        o.append(f'<polygon points="{poly(pg)}" fill="{sheath}" stroke="{a}" '
                 f'stroke-width="2" stroke-linejoin="round"/>')
        mx, my = seg[len(seg) // 2]                  # the Schwann cell's nucleus
        dx, dy = seg[-1][0] - seg[0][0], seg[-1][1] - seg[0][1]
        L = math.hypot(dx, dy) or 1.0
        o.append(f'<circle cx="{round(mx - dy / L * 3, 1)}" '
                 f'cy="{round(my + dx / L * 3, 1)}" r="1.7" fill="{a}" '
                 f'opacity="0.75"/>')

    o.append(f'<circle cx="{sx0 - 3}" cy="{sy0 - 1}" r="9" fill="{t["green"]}" '
             f'opacity="0.9"/>')
    o.append(f'<circle cx="{sx0 - 3}" cy="{sy0 - 1}" r="9" fill="none" '
             f'stroke="{t["green"]}" stroke-width="1.5"/>')
    o.append(f'<circle cx="{sx0 - 5}" cy="{sy0 - 3}" r="3.4" '
             f'fill="{mix(t["green"], t["value"], 0.45)}"/>')   # nucleolus

    for i, ch in enumerate(tips[::3]):
    ...
    o.append(f'<circle cx="{sx0 - 3}" cy="{sy0 - 1}" r="9" fill="{t["value"]}" '
    ...
    tgt = max(boutons, key=lambda b: b[1])       # the lowest bouton, out on its own
    o.append(f'<circle cx="{tgt[0]}" cy="{tgt[1]}" r="11" fill="none" '
    ...
    node, sleeve = axon[18], axon[35]            # a node of Ranvier, an internode
    top = min(boutons, key=lambda b: b[1])
    for lx, ly, anchor, label, tx, ty in (
        (60, 140, "start", "dendrites", 104, 184),
        (150, 126, "start", "soma", 140, 222),
        (60, 372, "start", "nucleus", 146, 250),
        (228, 236, "middle", "axon", axon[6][0], round(axon[6][1] - 6, 1)),
        (196, 382, "start", "node of Ranvier", node[0], round(node[1] + 7, 1)),
        (IX1, 172, "end", "myelin sheath", sleeve[0], round(sleeve[1] - 7, 1)),
        (IX1, 252, "end", "axon terminals", round(top[0] + 5, 1),
         round(top[1] - 5, 1)),
        (IX1, 392, "end", "synapse", round(tgt[0] + 9, 1), round(tgt[1] + 5, 1)),
    ):
